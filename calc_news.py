...
# Get last valid feature recored time from prediction time
# feature: feature name
# d: dictionary of feature list 
# pred_dt: prediction time
def get_latest_time(feature, d, pred_dt):
    if feature not in d:
        return NO_FEATURE_VALUE
...
def calc_oxygen_saturations(d, pred_dt):
    v = get_latest_value(SpO2, d, pred_dt)
    if v == NO_FEATURE_VALUE:
        return 0
...
# calculate supplemental oxygen score
def calc_supplemental_oxygen(d, pred_dt):
    fio2_time = get_latest_time(FiO2, d, pred_dt)
    o2_time = get_latest_time(O2, d, pred_dt)

    if (fio2_time == NO_FEATURE_VALUE) and (o2_time == NO_FEATURE_VALUE):
        return 0

    if (fio2_time != NO_FEATURE_VALUE) and (o2_time != NO_FEATURE_VALUE):
        t = max(fio2_time, o2_time)
    elif fio2_time != NO_FEATURE_VALUE:
        t = fio2_time
    else:
        t = o2_time

    if t < pred_dt - timedelta(days=1):
        return 0
    else:
        return 2
...
def calc_medscore(pred_time, features):
    ...
    medscore += calc_oxygen_saturations(features, pred_time)
    medscore += calc_supplemental_oxygen(features, pred_time)
